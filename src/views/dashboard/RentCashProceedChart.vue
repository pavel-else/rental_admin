  ...
  methods: {
    getStatAndRender() {
      ...
      const makeStatMonth = () => {
        ...
        this.stat = this.$store.getters.statistics({ cmd: 'getCashPerMonth', from: this.from, to: this.to, id: '8800000001' });
      ...
      const makeStatFromTo = () => {
        const period = shortDate.createPeriod(this.from, this.to);
        this.labels = period.map(i => i.slice(8, 12));
        this.stat = this.$store.getters.statistics({ cmd: 'getCashPerFromTo', from: this.from, to: this.to, id: '8800000001' });
      };
